# 🔍 B2B SaaS SEO AI Research

## Topic
**Producing SEO Content Based on Artificial Intelligence**

This repository documents my research into how top practitioners use AI to produce, optimize, and scale SEO content for B2B SaaS businesses.

---

## Why This Topic?

AI-powered SEO content is the fastest-moving area in digital marketing right now. B2B SaaS companies that master this will have a compounding advantage — more content, better targeting, lower cost per lead. I chose this topic because it sits at the intersection of two high-leverage skills: SEO strategy and AI tooling.

---

## Research Structure
research/

├── sources.md              # 10 experts with links and notes

├── linkedin-posts/         # LinkedIn content by author

├── youtube-transcripts/    # YouTube transcripts by video

└── other/                  # Additional documentation
---

## The 10 Experts

| # | Name | Why They Matter |
|---|------|----------------|
| 1 | Kevin Indig | Ex-Shopify SEO Director, deep AI SEO thinker |
| 2 | Eli Schwartz | Author of Product-Led SEO, B2B SaaS focus |
| 3 | Kyle Roof | Runs real AI SEO experiments via PageOptimizer Pro |
| 4 | Bernard Huang | Founder Clearscope, AI content optimization |
| 5 | Brendan Hufford | Tactical B2B SaaS SEO consultant |
| 6 | Cyrus Shepard | Founder Zyppy, ex-Moz, AI SEO tests |
| 7 | Mark Williams-Cook | Director Candour, public AI SEO experiments |
| 8 | Aleyda Solis | SEOFOMO newsletter, AI SEO workflows |
| 9 | Wil Reynolds | Founder Seer Interactive, AI + SEO strategy |
| 10 | Ethan Smith | CEO Graphite, AI content at scale for SaaS |

---

## Key Findings So Far

After analyzing 6 video transcripts and 4 LinkedIn posts from top AI SEO practitioners:

**Finding 1 — AI citations are topic-specific**
There is no universal list of sources AI trusts. For invoicing queries, competitor domains hold 33.5% of citations. For "starting a business" queries, only 7%. Generic PR campaigns miss entirely.

**Finding 2 — Traditional SEO still matters (for now)**
Analysis of 500,000+ AI overview citations shows strong correlation with organic rankings. But this overlap is decreasing as AI search matures.

**Finding 3 — Technical access is the foundation**
AI bots cannot render JavaScript. Many CDNs block AI crawlers by default. Before any content optimization, companies must validate that AI bots can actually access their site (free tool: Microsoft Clarity).

**Finding 4 — Passage-level beats page-level**
LLMs extract paragraphs, not pages. Each paragraph must be self-contained, one concept, semantically tight. This is the biggest shift from traditional SEO.

**Finding 5 — Measurement is broken**
Running a prompt once measures volatility, not visibility. Correct method: run each prompt 3-5x, report confidence intervals. Only 2.2% of citations remained consistent across 3 runs of the same prompt.
## Progress Log

- [x] Repository structure created
- [x] 10 experts identified and documented in `sources.md`
- [x] YouTube transcripts collected
- [x] LinkedIn posts collected
- [ ] Final analysis complete
